...
case_files <- c(
  ...
  "Outcome Shift" = "diffT_contrasts.RData",
  "Censoring Shift" = "diffC_contrasts.RData",
  "All Shifts" = "diffAll_contrasts.RData"
)
...
summarize_time_metric <- function(results, case_name, metric, true_value, M = 500) {
  ...
  out <- lapply(methods, function(m) {
    ...
    tibble(
      ...
      RRMSE = sqrt(colMeans(err^2, na.rm = TRUE)) / sqrt(colMeans(tgt_err^2, na.rm = TRUE)),
      CP = colMeans(sweep(est + 1.96 * sd, 2, true_value, ">") & sweep(est - 1.96 * sd, 2, true_value, "<"), na.rm = TRUE) * 100
    )
  })
  ...
  bind_rows(lapply(seq_len(nrow(rmst_info)), function(i) {
    ...
    bind_rows(lapply(methods, function(m) {
      ...
      tibble(
        ...
        CP = mean((true_value < est + 1.96 * sd) & (true_value > est - 1.96 * sd), na.rm = TRUE) * 100
      )
    }))
  }))
}
