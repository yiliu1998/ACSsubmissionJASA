...
all.cp <- unlist(CP.results(results, result.CLCOX.imbal))
n <- length(all.cp)
time <- rep(t, n/length(t))

Method <- factor(
  rep(c(rep("TGT", length(t)), rep("IVW", length(t)), rep("POOL", length(t)), 
        rep("CCOD", length(t)), rep("FED", length(t)), rep("CLCOX", length(t))), 2),
  levels = c("FED", "TGT", "IVW", "POOL", "CCOD", "CLCOX")
)

Treatment <- rep(c(rep("Treated", n/6), rep("Control", n/6)))

df <- data.frame(CP = all.cp, Method = Method, time = time, Treatment = Treatment)

df_heatmap <- df %>%
  group_by(Method, Treatment, time) %>%
  summarize(CP = mean(CP*100, na.rm=T), .groups='drop')
...
pdf("cp_imbal.pdf", width=3.5, height=5)
p.cp
dev.off()
...
all.cp <- unlist(CP.results(results, result.CLCOX.moreK, M=500))
n <- length(all.cp)
time <- rep(t, n/length(t))
...
pdf("cp_moreK.pdf", width=3.5, height=5)
p.cp
dev.off()
