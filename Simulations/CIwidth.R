...
all.ciw <- unlist(CIwidth.results(results, result.CLCOX.imbal, M=500))
n <- length(all.ciw)
...
df <- data.frame(CIW = all.ciw, Method = Method, time = time, Treatment = Treatment)

df_heatmap <- df %>%
  group_by(Method, Treatment, time) %>%
  summarize(CIW = median(CIW, na.rm=T), .groups = 'drop')
...
pdf(file = "ciw_imbal.pdf", width = 3.5, height = 5)
p.ciw
dev.off()
...
load("Res_CLCOX_imbal.Rdata")
all.ciw <- unlist(CIwidth.results(results, result.CLCOX.moreK, M=500))
n <- length(all.ciw)
time <- rep(rep(t, times = M), times = 2*6)
...
pdf(file = "ciw_moreK.pdf", width = 3.5, height = 5)
p.ciw
dev.off()
