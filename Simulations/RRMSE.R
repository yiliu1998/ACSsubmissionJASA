...
## Imbalanced sample size setting
load("Res_imbal_500.Rdata")
load("Res_CLCOX_imbal.Rdata")

df_heatmap <- RRMSE.results(results, result.CLCOX.imbal, M = 500, t = t)

p.rrmse <- ggplot(df_heatmap, aes(x = factor(time), y = Method, fill = RRMSE)) +
  geom_tile(color = "white") +
  geom_text(aes(label = round(RRMSE, 2)), size = 3, color = "black") +
  scale_fill_gradientn(
    colors = c("white", "white", "orange"),
...
df_heatmap <- RRMSE.results(results, result.CLCOX.moreK, M = 500, t = t)
